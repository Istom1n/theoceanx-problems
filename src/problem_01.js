...
const update = async() => {
     ...
     *
     * NB:
     * From StackOverflow: setTimeout() ensures that timer
     * events don't "stack" if they're left unprocessed.
     * In some circumstances a whole load of setInterval
     * events can arrive immediately after each other
     * without any delay. (c) Alnitak
     */
    setTimeout(update, 3600 * 1000)
}

const check_api = async() => {
    let last_5_hrs = await coinapi.ohlcv_historic_data(
        'POLONIEX_SPOT_ZRX_BTC',
        '1HRS',
        subHours(new Date(), 6),
        new Date()
    )
